...
b3(w, iiiter)  .. yita(w) =g= v(iiiter, w) + sum((j,p)$JP(j,p), g1(iiiter, j, p,w) * xf(j,p) + g2(iiiter, j,p, w) * capf(j,p) ) + sum((j,p)$JP(j,p), alphaC(j,p)*xf(j,p) + betaC(j,p)*capf(j,p) )  * prob(w);
...
* Objective functions:
  BSObjC..  ZC =e= sum(w$freeze(w), prob(w) *( CS(w) + CP(w) + CSP(w) + CPC(w)  + 900 * sum((j,c), Slack(j,c,w)))) ;
