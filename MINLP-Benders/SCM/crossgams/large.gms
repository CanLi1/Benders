...
Set 
subb/1*2/;
alias (sub0,sub1,sub2,sub3,sub4,sub5,sub6,sub7,sub8,sub9,sub10,sub11,subb);
loop(sub0,
 loop(sub1,
  loop(sub2,
   loop(sub3,
         ...
         loop(sub9,
          loop(sub10,
           loop(sub11,
            ...
            num = 1*(ord(sub0)-1)+2*(ord(sub1)-1)+4*(ord(sub2)-1)+8*(ord(sub3)-1)+16*(ord(sub4)-1)+32*(ord(sub5)-1)+64*(ord(sub6)-1)+128*(ord(sub7)-1)+256*(ord(sub8)-1)+512*(ord(sub9)-1)+1024*(ord(sub10)-1)+2048*(ord(sub11)-1);
            loop(w, 
              if(ord(w) eq num + 1,
            ...
            D(j,'C8', w )= tempD(j,'C8','2')*(1 + (ord(sub7)-1.5)/2);
            D(j,'C9', w )= tempD(j,'C9','2')*(1 + (ord(sub8)-1.5)/2);
            D(j,'C10', w )= tempD(j,'C10','2')*(1 + (ord(sub9)-1.5)/2);
            D(j,'C11', w )= tempD(j,'C11','2')*(1 + (ord(sub10)-1.5)/2);
            D(j,'C12', w )= tempD(j,'C12','2')*(1 + (ord(sub11)-1.5)/2);
            );
              );
...
b1(w, iiiter) .. yita(w) =g= obj_record(iiiter,w ) - sum((j,p)$JP(j,p), pix_all(iiiter,j,p,w) * xf(j,p)+ piCAP_all(iiiter,j,p,w)*capf(j,p));
b2(j,p)$JP(j,p) .. xf(j,p)*M(j,p)=g= capf(j,p);
b3(w, iiiter)  .. yita(w) =g= v(iiiter, w) + sum((j,p)$JP(j,p), g1(iiiter, j, p,w) * xf(j,p) + g2(iiiter, j,p, w) * capf(j,p) ) + sum((j,p)$JP(j,p), alphaC(j,p)*xf(j,p) + betaC(j,p)*capf(j,p) )  * prob;
