...
eobj .. cost =e= sum(p, sum(i, betaC(i) * QE(p,i) + alphaC(i) * x(p,i))) + sum(w, prob(w) * (sum((p,i,s,j)$(PS(i,s) and JM(i,s,j)), delta*rho(i,j,s) * theta(p,i,j,s,w)) + sum((p,j,r)$RJ(r,j), (betaS(r,j) + betaRP(r,p)) * PU(r,p,j,w)) + sum((r,p), alphaRP(r,p) * y(r,p,w)) + sum((p,c), alphaPC(p,c) * z(p,c,w)) + sum((p,c,j), betaPC(p,c) * F(p,c,j,w)) + sum((c,j), phi(c,j) * Slack(c,j,w)) ));
...
option optca =0;
  OPTION LIMROW = 0;
OPTION LIMCOL = 0;
solve scm using rmip minimizing cost;
parameters
cpu;
cpu = scm.resusd;
display x.l,y.l,z.l,PU.l, F.l, QE.l,theta.l,Q.l,WW.l,WWT.l,Slack.l, cpu;
