...
stepsize
theta0 /0.2/
*theta00(ltheta)/1 0.2, 2 0.4, 3 0.6, 4 0.8, 5 1, 6 1.2, 7 1.4, 8 1.6, 9 1.8, 10 2/
half0 /0.5/
*half00(lstep) / 1 0.4, 2 0.5,3 0.6,4 0.7, 5 0.8, 6 0.9/
change;
mux('1',p,i,w) = 0;
muQ('1',p,i,w)=0;
...
Equations
mle1,mle2,mle3,mle4,mle5,mle6,mle7,mle8,mle9;
mle1 .. zML =l= sum(w, kappa(w));
...
mle6(iiiter,p,i,w)$(ord(w) gt 1 and fiter(iiiter)) .. muxV(p,i,w) =l= mux(iiiter,p,i,w) + theta0 * (UB-LB) / den * abs(x_record_lag(iiiter,p,i,w) - x_record_lag(iiiter,p,i,'w1'));
mle7(iiiter,p,i,w)$(ord(w) gt 1 and fiter(iiiter)) .. muxV(p,i,w) =g= mux(iiiter,p,i,w) - theta0 * (UB-LB) / den * abs(x_record_lag(iiiter,p,i,w) - x_record_lag(iiiter,p,i,'w1'));
mle8(iiiter,p,i,w)$(ord(w) gt 1 and fiter(iiiter)) .. muQV(p,i,w) =l= muQ(iiiter,p,i,w) + theta0 * (UB-LB) / den * abs(Q_record_lag(iiiter,p,i,w) - Q_record_lag(iiiter,p,i,'w1'));
mle9(iiiter,p,i,w)$(ord(w) gt 1 and fiter(iiiter)) .. muQV(p,i,w) =g= muQ(iiiter,p,i,w) - theta0 * (UB-LB) / den * abs(Q_record_lag(iiiter,p,i,w) - Q_record_lag(iiiter,p,i,'w1'));
model ml /mle1,mle2,mle3,mle4,mle5,mle6,mle7,mle8,mle9/;
*
...
fiter(iter) = yes;
if(ord(iter) le 50, 
  den = sum((p,i,w), power(x_record_lag(iter,p,i,w) -x_record_lag(iter,p,i,'w1'), 2 ) + power(Q_record_lag(iter,p,i,w)-Q_record_lag(iter,p,i,'w1'), 2) );
  solve ml using lp maximizing zML;
...
display lag_sub_modelstat,bender_sub_modelstat,upper_sub_modelstat,master_modelstat,total_obj_record, BenderOBJ_record, UB_Bender,UB, LB, WallTime,cpu_ub,cpu_lag, cpu_bender_sub, cpu_bender_master , theta0;
