$ontext
the center value is fixed to the best multiplier we get using subgradient
method
$offtext
Sets
i /i1*i4/
...
p3.i3  0.00078671 -0.00208430  0.00129759
p3.i4 -0.00600718  0.00300359  0.00300359
;
...
p1.i1 -0.02508964 -0.04935173  0.07444137
p1.i2 -0.01744219 -0.04181596  0.05925815
p1.i3 -0.03542423 -0.05108190  0.08650612
p1.i4 -0.01522596  0.00534446  0.00988150
p2.i1 -0.03517023 -0.02498324  0.06015347
p2.i2 -0.02211314 -0.03849047  0.06060361
p2.i3 -0.04576731 -0.02683921  0.07260652
p2.i4 -0.01867832  0.00303960  0.01563872
p3.i1 -0.03393196 -0.03748252  0.07141448
p3.i2 -0.03224365 -0.02517649  0.05742013
p3.i3 -0.02247187 -0.08275244  0.10522430
p3.i4  0.01380266 -0.01445155  0.00064890;
Equations
...
para /d1*d8/;
Parameters
multpenalty(para) /d1 1000, d2 2000, d3 3000, d4 4000, d5 5000, d6 2500, d7 1500, d8 3500/
multpenalty0
zSB(iter, w)
...
mle1 .. zML =l= sum(w, kappa(w)) - multpenalty0*sum((p,i,w), power((muxV(p,i,w)-center_mux(p,i,w)), 2) + power((muQV(p,i,w)-center_muQ(p,i,w)), 2));
...
loop(w3$(ord(w3)gt 1),
mux('1',p,i,w3) = pix(p,i,w3);
muQ('1',p,i,w3)=piQ(p,i,w3);
center_mux(p,i,w3) = pix(p,i,w3);
center_muQ(p,i,w3)=piQ(p,i,w3);
);
...
until card(lag_sub_handle) =0;
  ...
  if(ord(iter) eq 1,
    last_best_LB = total_obj_record(iter);
    );
  if(total_obj_record( iter) - last_best_LB > 2,
...
display  mux, muQ,  total_obj_record, UB_Bender, LB,UB;
