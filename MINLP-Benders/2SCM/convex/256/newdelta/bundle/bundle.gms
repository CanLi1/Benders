...
para /d1*d3/;
Parameters
multpenalty(para) / d1 5e3, d2 8e3, d3 1.2e4/
multpenalty0
zUB(iter, w)
...
mle4(iiiter)$aiter(iiiter) .. kappa('w1') =l= zUB(iiiter, 'w1') + sum((p,i),sum(w$(ord(w) gt 1 ),  -muxV(p,i,w) ) * xf_record(iiiter, p,i) + sum(w$(ord(w) gt 1), -muQV(p,i,w)) * Qf_record(iiiter,p,i));
mle5(iiiter, w)$(aiter(iiiter) and ord(w) gt 1) .. kappa(w) =l= zUB(iiiter, w) + sum((p,i), muxV(p,i,w) * xf_record(iiiter, p,i) + muQV(p,i,w) * Qf_record(iiiter, p,i));
...
display  mux_record, muQ_record,LB_record, UB_record,  total_obj_record;
