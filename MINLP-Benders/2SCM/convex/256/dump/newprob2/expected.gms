     ...
     loop(sub5,
      loop(sub6,
        loop(sub7,
            ...
            loop(w, 
              if(ord(w) eq num + 1,
D('c1','j3',w)= baseD('c1', 'j3')*(1 + (ord(sub0)-1.5)/5);
D('c1','j5',w)= baseD('c1', 'j5')*(1 + (ord(sub1)-1.5)/5);
D('c2','j3',w)= baseD('c2', 'j3')*(1 + (ord(sub2)-1.5)/5);
D('c2','j5',w)= baseD('c2', 'j5')*(1 + (ord(sub3)-1.5)/5);
D('c3','j3',w)= baseD('c3', 'j3')*(1 + (ord(sub4)-1.5)/5);
D('c3','j5',w)= baseD('c3', 'j5')*(1 + (ord(sub5)-1.5)/5);
D('c4','j3',w)= baseD('c4', 'j3')*(1 + (ord(sub6)-1.5)/5);
D('c4','j5',w)= baseD('c4', 'j5')*(1 + (ord(sub7)-1.5)/5);
...
eobj .. cost =e= sum(w$freeze(w), prob(w) * (sum(p, sum(i, betaC(i) * QE(p,i) + alphaC(i) * x(p,i)))+sum((p,i,s,j)$(PS(i,s) and JM(i,s,j)), delta(i,s)*rho(i,j,s) * theta(p,i,j,s,w)) + sum((p,j,r)$RJ(r,j), (betaS(r,j) + betaRP(r,p)) * PU(r,p,j,w)) + sum((r,p), alphaRP(r,p) * y(r,p,w)) + sum((p,c), alphaPC(p,c) * z(p,c,w)) + sum((p,c,j), betaPC(p,c) * F(p,c,j,w)) + sum((c,j), phi(c,j) * Slack(c,j,w)) ));
...
prob('w257') = 1;
alias(w,www);
*get the expected demand
loop(w$w3(w),
  D(c,j,'w257') = D(c,j,'w257') + D(c,j,w) * prob(w); 
  );
D(c,j,'w257') = baseD(c,j);
