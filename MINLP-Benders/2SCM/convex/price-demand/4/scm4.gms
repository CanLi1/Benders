...
Equations
e1,e2,e3,e4,e5,e6,e7,e8,e9,e10,e11,e12,eobj,ec1,ec2,ec3,ec4;
...
eobj .. cost =e= sum(w$freeze(w), prob(w) * (sum(p, sum(i, betaC(i) * QE(p,i)*100 + alphaC(i) * x(p,i)))+sum((p,i,s,j)$(PS(i,s) and JM(i,s,j)), delta(i,s)*rho(i,j,s) * theta(p,i,j,s,w)) + sum((p,j,r)$RJ(r,j), (betaS(r,j,w) + betaRP(r,p)) * PU(r,p,j,w)) + sum((r,p), alphaRP(r,p) * y(r,p,w)) + sum((p,c), alphaPC(p,c) * z(p,c,w)) + sum((p,c,j), betaPC(p,c) * F(p,c,j,w)) + sum((c,j), phi(c,j) * Slack(c,j,w)) ));
ec1 .. c1 =e=sum(w$freeze(w), prob(w) * sum((p,i,s,j)$(PS(i,s) and JM(i,s,j)), delta(i,s)*rho(i,j,s) * theta(p,i,j,s,w)));
ec2 .. c2 =e= sum(w$freeze(w), prob(w) * (sum((p,j,r)$RJ(r,j), (betaS(r,j,w) + betaRP(r,p)) * PU(r,p,j,w))+sum((r,p), alphaRP(r,p) * y(r,p,w))));
ec3 .. c3 =e= sum(w$freeze(w), prob(w) * (sum((p,c), alphaPC(p,c) * z(p,c,w)) + sum((p,c,j)$(ord(j) ne 5), betaPC(p,c) * F(p,c,j,w))+ sum((p,c,j)$(ord(j) eq 5), betaPC(p,c) * F(p,c,j,w))));
ec4 .. c4 =e= sum(w$freeze(w), prob(w) * (sum(c, phi(c,'j3') * Slack(c,'j3',w))+sum(c, phi(c,'j5')*Slack(c,'j5',w))));
model sub/e1,e2,e3,e4,e5,e6,e7,e8,e9,e10,e11,e12,ec1,ec2,ec3,ec4,eobj/;
sub.optfile=1;
freeze(w)=yes;
options decimals=6;
options optca = 0;
...
display Q.l,Slack.l, cost.l,c1.l,c2.l,c3.l,c4.l,walltime;
