...
Equations
e1,e2,e3,e4,e5,e6,e7,e8,e9,e10,e10p,e11,e12,eobj,ec1,ec2,ec3,ec4;
...
e8(p,i,j,jj,w,s)$(freeze(w) and Lbar(i,s,j) and PS(i,s) and JM(i,s,jj)) .. log(1+WW(p,i,j,s,w)*100) =g=  mu(i,s,j) * WW(p,i,jj,s,w);
e9(r,p,w,j)$(RJ(r,j) and freeze(w)) .. PU(r,p,j,w) =l= PUU * y(r,p,w);
e10(p,c,j,w)$(freeze(w) and ord(j) ne 5) .. F(p,c,j,w) =l= FUU * z(p,c,w);
e10p(p,c,j,w)$(freeze(w) and ord(j) eq 5) .. F(p,c,j,w) =l= 150 * z(p,c,w);
...
eobj .. cost =e= sum(w$freeze(w), prob(w) * (sum(p, sum(i, betaC(i) * QE(p,i)*100 + alphaC(i) * x(p,i)))+sum((p,i,s,j)$(PS(i,s) and JM(i,s,j)), delta(i,s)*rho(i,j,s) * theta(p,i,j,s,w)*100) + sum((p,j,r)$RJ(r,j), (betaS(r,j,w) + betaRP(r,p)) * PU(r,p,j,w)*100) + sum((r,p), alphaRP(r,p) * y(r,p,w)) + sum((p,c), alphaPC(p,c) * z(p,c,w)) + sum((p,c,j)$(ord(j) ne 5), betaPC(p,c) * F(p,c,j,w)*100)  + sum((p,c,j)$(ord(j) eq 5), betaPC(p,c) * F(p,c,j,w))+sum(c, phi(c,'j3') * Slack(c,'j3',w)*100)+sum(c, phi(c,'j5')*Slack(c,'j5',w)) ));
ec1 .. c1 =e=sum(w$freeze(w), prob(w) * sum((p,i,s,j)$(PS(i,s) and JM(i,s,j)), delta(i,s)*rho(i,j,s) * theta(p,i,j,s,w)*100));
ec2 .. c2 =e= sum(w$freeze(w), prob(w) * (sum((p,j,r)$RJ(r,j), (betaS(r,j,w) + betaRP(r,p)) * PU(r,p,j,w)*100)+sum((r,p), alphaRP(r,p) * y(r,p,w))));
ec3 .. c3 =e= sum(w$freeze(w), prob(w) * (sum((p,c), alphaPC(p,c) * z(p,c,w)) + sum((p,c,j)$(ord(j) ne 5), betaPC(p,c) * F(p,c,j,w)*100)+ sum((p,c,j)$(ord(j) eq 5), betaPC(p,c) * F(p,c,j,w))));
ec4 .. c4 =e= sum(w$freeze(w), prob(w) * (sum(c, phi(c,'j3') * Slack(c,'j3',w)*100)+sum(c, phi(c,'j5')*Slack(c,'j5',w))));
model sub/e1,e2,e3,e4,e5,e6,e7,e8,e9,e10,e11,e12,eobj,ec1,ec2,ec3,ec4/;
sub.optfile=1;
freeze(w)=yes;
options optca = 0;
options optcr =0;
...
display WW.L,Slack.l,Q.l, c1.l,c2.l,c3.l,c4.l,cost.l,walltime;
