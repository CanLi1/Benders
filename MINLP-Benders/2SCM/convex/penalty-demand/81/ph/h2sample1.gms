...
eobj .. cost =e= sum(w$freeze(w), prob(w) * sum(p, sum(i, betaC(i) * QE(p,i)*100 + alphaC(i) * x(p,i)))) + sum(w$freeze(w), prob(w) * (sum((p,i,s,j)$(PS(i,s) and JM(i,s,j)), delta(i,s)*rho(i,j,s) * theta(p,i,j,s,w)) + sum((p,j,r)$RJ(r,j), (betaS(r,j) + betaRP(r,p)) * PU(r,p,j,w)) + sum((r,p), alphaRP(r,p) * y(r,p,w)) + sum((p,c), alphaPC(p,c) * z(p,c,w)) + sum((p,c,j), betaPC(p,c) * F(p,c,j,w)) + sum((c,j), phi(c,j,w) * Slack(c,j,w)) )) +  sum((p,i,w)$freeze(w), prob(w) * (pix(p,i,w) * x(p,i) + piQ(p,i,w) * Q(p,i)))+   sum((p,i,w)$freeze(w), prob(w) * penalty_x(i)/2*power((x(p,i)-avg_x(p,i)), 2) +prob(w) *  penalty_Q(p,i)/2*power((Q(p,i)-avg_Q(p,i)), 2)) ;
set iter /1*30/
aiter(iter)
biter(iter);
...
parameters
xbar(p,i), Qbar(p,i), UB_Bender(iter), cpu_bender_sub/0/, cpu_bender_master /0/, cpu_ub/0/, cpu_ph/0/,UB_Bender(iter) ;
UB_Bender(iter) = 0;
