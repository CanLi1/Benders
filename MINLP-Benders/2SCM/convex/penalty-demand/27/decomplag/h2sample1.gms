...
avg_x(p,i) = sum(w3, prob(w3) * x_record_lag(iter, p,i, w3));
avg_Q(p,i) = sum(w3, prob(w3) * Q_record_lag(iter, p,i,w3));
distance = 1e6;
loop(w3,
  temp_distance = sqrt(sum((p,i), power((avg_x(p,i)- x_record_lag(iter, p,i, w3)), 2) + power((avg_Q(p,i) - Q_record_lag(iter,p,i, w3)), 2)));
  if(temp_distance lt distance,
    xbar(p,i) = x_record_lag(iter, p,i, w3);
    Qbar(p,i) = Q_record_lag(iter, p,i, w3);
...
if(ord(iter) le 30,
  den(iter) = sum((p,i,w), power(x_record_lag(iter,p,i,w) -x_record_lag(iter,p,i,'w14'), 2 ) + power(Q_record_lag(iter,p,i,w)-Q_record_lag(iter,p,i,'w14'), 2) );
  stepsize = theta0 * (UB-LB)/den(iter);
  loop(w,
    if(ord(w) ne 14,
      mux(iter+1,p,i,w) = mux(iter,p,i,w) + (x_record_lag(iter,p,i,w)-x_record_lag(iter,p,i,'w14')) * stepsize;
      muQ(iter+1,p,i,w) = muQ(iter,p,i,w) + (Q_record_lag(iter,p,i,w)-Q_record_lag(iter,p,i,'w14')) * stepsize;
      );
    );
  loop(w,
    if(ord(w) ne 14,
    ...
    pix(p,i,'w14') = -sum((w2)$( ord(w2) ne 14), mux(iter+1,p,i,w2));
    piQ(p,i,'w14') = -sum((w2)$( ord(w2) ne 14 ), muQ(iter+1,p,i,w2));
    );
  if (ord(iter) ne 1,
