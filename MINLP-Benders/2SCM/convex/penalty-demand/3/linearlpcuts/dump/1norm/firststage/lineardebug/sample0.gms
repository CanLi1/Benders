...
piQ(p,i,w) = 0;
Equations
e1,e2,e3,e4,e5,e6,e7,e8,le8,e9,e10,e11,e12,eobj;
...
le8(p,i,j,jj,w,s, int)$(freeze(w) and Lbar(i,s,j) and PS(i,s) and JM(i,s,jj)) .. -log(1+intWW(int)) +(-1/(1+intWW(int)))*( WW(p,i,j,s,w)-intWW(int)) + mu(i,s,j) * WW(p,i,jj,s,w) =l=0;
...
model sub/e1,e2,e3,e4,e5,e6,e7,le8,e9,e10,e11,e12,eobj/;
sub.optfile=1;
...
model Bendersub /TX, TQ,e1,e2,e3,e4,e5,e6,e7,le8,e9,e10,e11,e12,Beobj/;
BenderSub.optfile=1;
...
equations
f1,f2,f3,f4,d1, d2,d3,d5,d6,d7,d8,d9,d10,d11,d12,d12p,d14p,d14,d15,d16,d17,de3,de4,de5,de6,de7,de8,de9,de10,de11,de12,ds1,ds2,ds3,ds4,dobj;
...
n9p(p,i) .. ax(p,i) =g= lpx(p,i) - xbar(p,i);
n9m(p,i) .. ax(p,i) =g= xbar( p,i ) - lpx(p,i);
n10p(p,i) .. aQE(p,i) =g= lpQE(p,i) - Qbar( p,i);
n10m(p,i) .. aQE(p,i) =g= Qbar( p,i) - lpQE(p,i);
...
model sep /f1,f2,f3,f4,d1, d2,d3,d5,d6,d7,d8,d9,d10,d11,d12,d12p,d14p,d14,d15,d16,d17,de3,de5,de6,de7,de8,de9,de10,de11,de12,ds1,ds2,ds3,ds4,dobj,n1p,n1m,n2p,n2m,n3p,n3m,n4p,n4m,n5p,n5m,n6p,n6m,n7p,n7m,n8p,n8m, n9p, n9m, n10p, n10m/;
...
xrp(p,i,w,rr,pp)  =0;
QErp(p,i, w, rr,pp) =0;
PUrp(r,p,j,w,rr,pp) =0;
Frp(p,c,j,w,rr,pp)  =0;
thetarp(p,i,j,s,w,rr,pp)  =0;
WWrp(p,i,j,s,w,rr,pp) =0;
Slackrp(c,j,w,rr,pp)  =0;
yrp(r,p,w,rr,pp)  =0;
zrp(p,c,w,rr,pp)  =0;
xpc( p,i,w,pp,cc) =0;
QEpc( p,i,w,pp,cc)  =0;
PUpc(r,p,j,w,pp,cc) =0;
Fpc(p,c,j,w,pp,cc)  =0;
thetapc(p,i,j,s,w,pp,cc)  =0;
WWpc(p,i,j,s,w,pp,cc) =0;
Slackpc(c,j,w,pp,cc)  =0;
ypc(r,p,w,pp,cc)  =0;
zpc(p,c,w,pp,cc)  =0;
...
model lpsub /TX, TQ,e1,e2,e3,e4,e5,e6,e7,le8,e9,e10,e11,e12,Beobj, lp1,lp2/;
...
*update parameters
  xrp( p,i, w3, r3, p3) = lpx.l(p,i);
  QErp(p,i,w3, r3,p3) = lpQE.l(p,i);
  ...
  loop(c3,
    ...
    Repeat
      loop(w3$handlecollect(sep_handle(w3)),
        cpu_sep = cpu_sep + sep.resusd;
*update parameters
    xpc(p,i,w3, p3,c3) = lpx.l(p,i);
    QEpc( p,i,w3, p3,c3) = lpQE.l(p,i);
