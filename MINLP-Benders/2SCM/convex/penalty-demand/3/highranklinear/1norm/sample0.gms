...
PUhat(r,p,j,w, rank)
Fhat(p,c,j,w, rank)
thetahat(p,i,j,s,w, rank)
WWhat(p,i,j,s,w, rank)
Slackhat(c,j,w, rank)
yhat(r,p,w, rank)
zhat(p,c,w, rank)
epsilon /1e-4/;
...
parameters
PUrp(r,p,j,w,rr,pp, rank)
Frp(p,c,j,w,rr,pp, rank)
thetarp(p,i,j,s,w,rr,pp, rank)
WWrp(p,i,j,s,w,rr,pp, rank)
Slackrp(c,j,w,rr,pp, rank)
yrp(r,p,w,rr,pp, rank)
zrp(p,c,w,rr,pp, rank)
PUpc(r,p,j,w,pp,cc, rank)
Fpc(p,c,j,w,pp,cc, rank)
thetapc(p,i,j,s,w,pp,cc, rank)
WWpc(p,i,j,s,w,pp,cc, rank)
Slackpc(c,j,w,pp,cc, rank)
ypc(r,p,w,pp,cc, rank)
zpc(p,c,w,pp,cc, rank)
;


*DEFINE equations for CGNLP
equations
d1, d2,d3,d5,d6,d7,d8,d9,d10,d11,d12,d12p,d14p,d14,d15,d16,d17,de3,de4,de5,de6,de7,de8,de9,de10,de11,de12,ds1,ds2,ds3,ds4,dobj, delp1,delp2;
...
*the lower rank lift-and-project cuts
delp1(rr,pp,w,djc, rank)$(freeze(w) and aRP(rr,pp,w, rank) and arank(rank)) .. sum((r,p,j)$RJ(r,j), sign((PUrp(r,p,j,w,rr,pp, rank)-PUhat(r,p,j,w, rank))/PUU)*(vPU(r,p,j,djc, w) - PUrp(r,p,j,w,rr,pp, rank))/PUU) + sum((p,c,j), sign((Frp(p,c,j,w,rr,pp, rank)-Fhat(p,c,j,w, rank))/FUU)*(vF(p,c,j,djc, w)-Frp(p,c,j,w,rr,pp, rank))/FUU) + sum((p,i,j,s)$(JM(i,s,j) and PS(i,s)and (ord(i) ne 4 or ord(j) ne 5 )), sign((thetarp(p,i,j,s,w,rr,pp, rank)-thetahat(p,i,j,s,w, rank))/QEU(p,i)/100)*(vtheta(p,i,j,s,djc, w) - thetarp(p,i,j,s,w,rr,pp, rank))/QEU(p,i)/100) + sum((p,i,j,s)$(JM(i,s,j) and PS(i,s)and (ord(i) eq 4 and ord(j) eq 5 )), sign((thetarp(p,i,j,s,w,rr,pp, rank)-thetahat(p,i,j,s,w, rank))/QEU(p,i)/5)*(vtheta(p,i,j,s,djc, w) - thetarp(p,i,j,s,w,rr,pp, rank))/QEU(p,i)/5)+ sum((p,i,j,s)$((L(i,s,j) or Lbar(i,s,j)) and PS(i,s)), sign((WWrp(p,i,j,s,w,rr,pp, rank)-WWhat(p,i,j,s,w, rank))/QEU(p,i)/100)*(vWW(p,i,j,s,djc, w)-WWrp(p,i,j,s,w,rr,pp, rank))/QEU(p,i)/100) + sum((c,j)$(ord(j)=3 or ord(j)=5), sign((Slackrp(c,j,w,rr,pp, rank)-Slackhat(c,j,w, rank))/D(c,j,w))*(vSlack(c,j,djc,w) - Slackrp(c,j,w,rr,pp, rank))/D(c,j,w)) + sum((r,p), sign((yrp(r,p,w,rr,pp, rank)-yhat(r,p,w, rank)))*(vy(r,p,djc,w)-yrp(r,p,w,rr,pp, rank))) +sum((p,c), sign((zrp(p,c,w,rr,pp, rank)-zhat(p,c,w, rank)))*(vz(p,c,djc,w)-zrp(p,c,w,rr,pp, rank))) =g= 0;
delp2(pp,cc,w, djc, rank)$(freeze(w) and aPC(pp,cc,w, rank) and arank(rank)) .. sum((r,p,j)$RJ(r,j), sign((PUpc(r,p,j,w,pp,cc, rank)-PUhat(r,p,j,w, rank))/PUU)*(vPU(r,p,j,djc,w) - PUpc(r,p,j,w,pp,cc,rank))/PUU) + sum((p,c,j), sign((Fpc(p,c,j,w,pp,cc, rank)-Fhat(p,c,j,w, rank))/FUU)*(vF(p,c,j,djc,w)-Fpc(p,c,j,w,pp,cc, rank))/FUU) + sum((p,i,j,s)$(JM(i,s,j) and PS(i,s)and (ord(i) ne 4 or ord(j) ne 5 )), sign((thetapc(p,i,j,s,w,pp,cc, rank)-thetahat(p,i,j,s,w, rank))/QEU(p,i)/100)*(vtheta(p,i,j,s,djc,w) - thetapc(p,i,j,s,w,pp,cc, rank))/QEU(p,i)/100)+ sum((p,i,j,s)$(JM(i,s,j) and PS(i,s)and (ord(i) eq 4 and ord(j) eq 5 )), sign((thetapc(p,i,j,s,w,pp,cc, rank)-thetahat(p,i,j,s,w, rank))/QEU(p,i)/5)*(vtheta(p,i,j,s,djc,w) - thetapc(p,i,j,s,w,pp,cc, rank))/QEU(p,i)/5)+ sum((p,i,j,s)$((L(i,s,j) or Lbar(i,s,j)) and PS(i,s)), sign((WWpc(p,i,j,s,w,pp,cc, rank)-WWhat(p,i,j,s,w, rank))/QEU(p,i)/100)*(vWW(p,i,j,s,djc,w)-WWpc(p,i,j,s,w,pp,cc, rank))/QEU(p,i)/100) + sum((c,j)$(ord(j)=3 or ord(j)=5), sign((Slackpc(c,j,w,pp,cc, rank)-Slackhat(c,j,w, rank))/D(c,j,w))*(vSlack(c,j,djc,w) - Slackpc(c,j,w,pp,cc, rank))/D(c,j,w)) + sum((r,p), sign((ypc(r,p,w,pp,cc, rank)-yhat(r,p,w, rank)))*(vy(r,p,djc,w)-ypc(r,p,w,pp,cc, rank))) +sum((p,c), sign((zpc(p,c,w,pp,cc, rank)-zhat(p,c,w, rank)))*(vz(p,c,djc,w)-zpc(p,c,w,pp,cc, rank))) =g= 0;
...
n1p(r,p,j,w,rank)$(freeze(w) and brank(rank)) .. aPU(r,p,j,w) =g= (lpPU(r,p,j,w) - PUhat(r,p,j,w, rank))/PUU;
n1m(r,p,j,w,rank)$(freeze(w) and brank(rank)) .. aPU(r,p,j,w) =g= -(lpPU(r,p,j,w) - PUhat(r,p,j,w, rank))/PUU;
n2p(p,c,j,w,rank)$(freeze(w) and brank(rank)) .. aF(p,c,j,w) =g= (lpF(p,c,j,w)-Fhat(p,c,j,w, rank))/FUU;
n2m(p,c,j,w,rank)$(freeze(w) and brank(rank)) .. aF(p,c,j,w) =g= -(lpF(p,c,j,w)-Fhat(p,c,j,w, rank))/FUU;
n3p(p,i,j,s,w,rank)$(freeze(w) and JM(i,s,j) and PS(i,s) and brank(rank) and (ord(i) ne 4 or ord(j) ne 5 )) .. atheta(p,i,j,s,w)=g= (lptheta(p,i,j,s,w) - thetahat(p,i,j,s,w, rank))/QEU(p,i)/100;
n3m(p,i,j,s,w,rank)$(freeze(w) and JM(i,s,j) and PS(i,s) and brank(rank) and (ord(i) ne 4 or ord(j) ne 5 )) .. atheta(p,i,j,s,w)=g= -(lptheta(p,i,j,s,w) - thetahat(p,i,j,s,w, rank))/QEU(p,i)/100;
n4p(p,i,j,s,w,rank)$(freeze(w) and JM(i,s,j) and PS(i,s) and brank(rank) and (ord(i) eq 4 and ord(j) eq 5 )) .. atheta(p,i,j,s,w)=g= (lptheta(p,i,j,s,w) - thetahat(p,i,j,s,w, rank))/QEU(p,i)/5;
n4m(p,i,j,s,w,rank)$(freeze(w) and JM(i,s,j) and PS(i,s) and brank(rank) and (ord(i) eq 4 and ord(j) eq 5 )) .. atheta(p,i,j,s,w)=g= -(lptheta(p,i,j,s,w) - thetahat(p,i,j,s,w, rank))/QEU(p,i)/5;
n5p(p,i,j,s,w,rank)$(freeze(w) and (L(i,s,j) or Lbar(i,s,j)) and PS(i,s) and brank(rank)) .. aWW(p,i,j,s,w) =g= (lpWW(p,i,j,s,w)-WWhat(p,i,j,s,w, rank))/QEU(p,i)/100;
n5m(p,i,j,s,w,rank)$(freeze(w) and (L(i,s,j) or Lbar(i,s,j)) and PS(i,s) and brank(rank) ) .. aWW(p,i,j,s,w) =g= -(lpWW(p,i,j,s,w)-WWhat(p,i,j,s,w, rank))/QEU(p,i)/100;
n6p(c,j,w,rank)$(freeze(w) and (ord(j)=3 or ord(j)=5) and brank(rank)) .. aSlack(c,j,w) =g= (Slack(c,j,w) - Slackhat(c,j,w,rank))/D(c,j,w);
n6m(c,j,w,rank)$(freeze(w) and (ord(j)=3 or ord(j)=5) and brank(rank)) .. aSlack(c,j,w) =g= -(Slack(c,j,w) - Slackhat(c,j,w,rank))/D(c,j,w);
n7p(r,p,w,rank)$(freeze(w) and brank(rank)) .. ay(r,p,w) =g= lpy(r,p,w)-yhat(r,p,w,rank);
n7m(r,p,w,rank)$(freeze(w) and brank(rank)).. ay(r,p,w) =g= -lpy(r,p,w)+yhat(r,p,w,rank);
n8p(p,c,w,rank)$(freeze(w) and brank(rank)) .. az(p,c,w) =g= (lpz(p,c,w)-zhat(p,c,w,rank));
n8m(p,c,w,rank)$(freeze(w) and brank(rank)) .. az(p,c,w) =g= -(lpz(p,c,w)-zhat(p,c,w,rank));
...
equations
lp1, lp2;
lp1(rr,pp,w, rank)$(freeze(w) and aRP(rr,pp,w, rank) and arank(rank)) .. sum((r,p,j)$RJ(r,j), sign((PUrp(r,p,j,w,rr,pp, rank)-PUhat(r,p,j,w, rank))/PUU)*(PU(r,p,j,w) - PUrp(r,p,j,w,rr,pp, rank))/PUU) + sum((p,c,j), sign((Frp(p,c,j,w,rr,pp, rank)-Fhat(p,c,j,w, rank))/FUU)*(F(p,c,j,w)-Frp(p,c,j,w,rr,pp, rank))/FUU) + sum((p,i,j,s)$(JM(i,s,j) and PS(i,s)and (ord(i) ne 4 or ord(j) ne 5 )), sign((thetarp(p,i,j,s,w,rr,pp, rank)-thetahat(p,i,j,s,w, rank))/QEU(p,i)/100)*(theta(p,i,j,s,w) - thetarp(p,i,j,s,w,rr,pp, rank))/QEU(p,i)/100) + sum((p,i,j,s)$(JM(i,s,j) and PS(i,s)and (ord(i) eq 4 and ord(j) eq 5 )), sign((thetarp(p,i,j,s,w,rr,pp, rank)-thetahat(p,i,j,s,w, rank))/QEU(p,i)/5)*(theta(p,i,j,s,w) - thetarp(p,i,j,s,w,rr,pp, rank))/QEU(p,i)/5)+ sum((p,i,j,s)$((L(i,s,j) or Lbar(i,s,j)) and PS(i,s)), sign((WWrp(p,i,j,s,w,rr,pp, rank)-WWhat(p,i,j,s,w, rank))/QEU(p,i)/100)*(WW(p,i,j,s,w)-WWrp(p,i,j,s,w,rr,pp, rank))/QEU(p,i)/100) + sum((c,j)$(ord(j)=3 or ord(j)=5), sign((Slackrp(c,j,w,rr,pp, rank)-Slackhat(c,j,w, rank))/D(c,j,w))*(Slack(c,j,w) - Slackrp(c,j,w,rr,pp, rank))/D(c,j,w)) + sum((r,p), sign((yrp(r,p,w,rr,pp, rank)-yhat(r,p,w, rank)))*(y(r,p,w)-yrp(r,p,w,rr,pp,rank))) +sum((p,c), sign((zrp(p,c,w,rr,pp, rank)-zhat(p,c,w, rank)))*(z(p,c,w)-zrp(p,c,w,rr,pp, rank))) =g= 0;
lp2(pp,cc,w, rank)$(freeze(w) and aPC(pp,cc,w, rank) and arank(rank)) .. sum((r,p,j)$RJ(r,j), sign((PUpc(r,p,j,w,pp,cc, rank)-PUhat(r,p,j,w, rank))/PUU)*(PU(r,p,j,w) - PUpc(r,p,j,w,pp,cc, rank))/PUU) + sum((p,c,j), sign((Fpc(p,c,j,w,pp,cc, rank)-Fhat(p,c,j,w, rank))/FUU)*(F(p,c,j,w)-Fpc(p,c,j,w,pp,cc, rank))/FUU) + sum((p,i,j,s)$(JM(i,s,j) and PS(i,s)and (ord(i) ne 4 or ord(j) ne 5 )), sign((thetapc(p,i,j,s,w,pp,cc, rank)-thetahat(p,i,j,s,w, rank))/QEU(p,i)/100)*(theta(p,i,j,s,w) - thetapc(p,i,j,s,w,pp,cc, rank))/QEU(p,i)/100)+ sum((p,i,j,s)$(JM(i,s,j) and PS(i,s)and (ord(i) eq 4 and ord(j) eq 5 )), sign((thetapc(p,i,j,s,w,pp,cc, rank)-thetahat(p,i,j,s,w, rank))/QEU(p,i)/5)*(theta(p,i,j,s,w) - thetapc(p,i,j,s,w,pp,cc, rank))/QEU(p,i)/5)+ sum((p,i,j,s)$((L(i,s,j) or Lbar(i,s,j)) and PS(i,s)), sign((WWpc(p,i,j,s,w,pp,cc, rank)-WWhat(p,i,j,s,w, rank))/QEU(p,i)/100)*(WW(p,i,j,s,w)-WWpc(p,i,j,s,w,pp,cc, rank))/QEU(p,i)/100) + sum((c,j)$(ord(j)=3 or ord(j)=5), sign((Slackpc(c,j,w,pp,cc, rank)-Slackhat(c,j,w, rank))/D(c,j,w))*(Slack(c,j,w) - Slackpc(c,j,w,pp,cc, rank))/D(c,j,w)) + sum((r,p), sign((ypc(r,p,w,pp,cc, rank)-yhat(r,p,w, rank)))*(y(r,p,w)-ypc(r,p,w,pp,cc, rank))) +sum((p,c), sign((zpc(p,c,w,pp,cc, rank)-zhat(p,c,w, rank)))*(z(p,c,w)-zpc(p,c,w,pp,cc, rank))) =g= 0;
model lpsub /TX, TQ,e1,e2,e3,e4,e5,e6,e7,e8,e9,e10,e11,e12,Beobj, lp1,lp2/;
aRP(rr,pp,w, rank) = no;
aPC(pp,cc,w, rank) = no;
PUhat(r,p,j,w, rank)   =0;
Fhat(p,c,j,w, rank)  =0;
thetahat(p,i,j,s,w, rank)  =0;
WWhat(p,i,j,s,w, rank)   =0;
Slackhat(c,j,w, rank)  =0;
yhat(r,p,w, rank)  =0;
zhat(p,c,w, rank)  =0;
PUrp(r,p,j,w,rr,pp, rank)  =0;
Frp(p,c,j,w,rr,pp, rank)   =0;
thetarp(p,i,j,s,w,rr,pp, rank)   =0;
WWrp(p,i,j,s,w,rr,pp, rank)  =0;
Slackrp(c,j,w,rr,pp, rank)   =0;
yrp(r,p,w,rr,pp, rank)   =0;
zrp(p,c,w,rr,pp, rank)   =0;
PUpc(r,p,j,w,pp,cc, rank)  =0;
Fpc(p,c,j,w,pp,cc, rank)   =0;
thetapc(p,i,j,s,w,pp,cc, rank)   =0;
WWpc(p,i,j,s,w,pp,cc, rank)  =0;
Slackpc(c,j,w,pp,cc, rank)   =0;
ypc(r,p,w,pp,cc, rank)   =0;
zpc(p,c,w,pp,cc, rank)   =0;
*-------------------solve model -----------------------------------
option optcr = 0;
option optca =0;
  OPTION LIMROW = 0;
...
*start solving problems with lift-and- project cuts 
arank(rank) = no;
loop(rank1,
  brank(rank) = no;
  brank(rank1) = yes;
*solve subproblem with lift and project cuts--------------
loop(w3,
  freeze(w2) = no;
  freeze(w3) = yes;
  ...
  loop(w3$handlecollect(lpsub_handle(w3)),
    ...
    if(ord(rank1) eq 1,
      bender_sub_obj(iter, w3) = cost.l;
        ...
        if(lpsub.modelStat ne 1 and lpsub.modelStat ne 2 ,
    baditer(iter) = yes;
    biter(iter) = no;
    );
*update parameters
      PUhat(r,p,j,w3, rank1) = PU.l(r,p,j,w3);
    Fhat(p,c,j,w3, rank1) = F.l(p,c,j,w3);
    thetahat(p,i,j,s,w3, rank1) = theta.l(p,i,j,s,w3);
    WWhat(p,i,j,s,w3, rank1) = WW.l(p,i,j,s,w3);
    Slackhat(c,j,w3, rank1) = Slack.l(c,j,w3);
    yhat(r,p,w3, rank1) = y.l(r,p,w3);
    zhat(p,c,w3, rank1) = z.l(p,c, w3);
...
loop(r3,
  loop(p3,
    ...
    loop(w3,
      freeze(w2) = no;
      freeze(w3) = yes;
      if(yhat(r3,p3,w3, rank1) gt 1e-1 and yhat(r3,p3,w3, rank1) lt (1 -1e-1),
        aRP(r3,p3,w3,rank1) = yes;
...
*update parameters
          PUrp(r,p,j,w3,r3,p3, rank1) = lpPU.l(r,p,j,w3);
        Frp(p,c,j,w3,r3,p3, rank1) = lpF.l(p,c,j,w3);
        thetarp(p,i,j,s,w3,r3,p3, rank1) = lptheta.l(p,i,j,s,w3);
        WWrp(p,i,j,s,w3,r3,p3, rank1) = lpWW.l(p,i,j,s,w3);
        Slackrp(c,j,w3,r3,p3, rank1) = lpSlack.l(c,j,w3);
        yrp(r,p,w3,r3,p3, rank1) = lpy.l(r,p,w3);
        zrp(p,c,w3,r3,p3, rank1) = lpz.l(p,c, w3);
...
*       abort$(sep.modelStat ne 2 and sep.modelStat ne 1 and sep.modelStat ne 7) "abort due to errors solving sep problem";
       display$handledelete(sep_handle(w3)) 'trouble deleting handles';
        if(dnorm.l lt 1e-2 or (sep.modelStat ne 2 and sep.modelStat ne 1),
          aRP(r3,p3,w3, rank1) = no;
          );
...
loop(p3,
  loop(c3,
    ...
    loop(w3,
      freeze(w2) = no;
      freeze(w3) = yes;
      if(zhat(p3,c3,w3, rank1) gt 1e-1 and zhat(p3,c3,w3, rank1) lt (1 -1e-1),
        aPC(p3,c3,w3, rank1) = yes;
        solve sep using rMIP minimizing dnorm;
        sep_handle(w3) = sep.handle;
        );
      );
    Repeat
      loop(w3$handlecollect(sep_handle(w3)),
        cpu_sep = cpu_sep + sep.resusd;
*update parameters
          PUpc(r,p,j,w3,p3,c3, rank1) = lpPU.l(r,p,j,w3);
        Fpc(p,c,j,w3,p3,c3, rank1) = lpF.l(p,c,j,w3);
        thetapc(p,i,j,s,w3,p3,c3, rank1) = lptheta.l(p,i,j,s,w3);
        WWpc(p,i,j,s,w3,p3,c3, rank1) = lpWW.l(p,i,j,s,w3);
        Slackpc(c,j,w3,p3,c3, rank1) = lpSlack.l(c,j,w3);
        ypc(r,p,w3,p3,c3, rank1) = lpy.l(r,p,w3);
        zpc(p,c,w3,p3,c3, rank1) = lpz.l(p,c, w3);
        display sep.modelStat;
        if(dnorm.l lt 1e-2 or (sep.modelStat ne 2 and sep.modelStat ne 1),
          aPC(p3,c3,w3, rank1) = no;
          );
*       abort$(sep.modelStat ne 2 and sep.modelStat ne 1 and sep.modelStat ne 7) "abort due to errors solving sep problem";
       display$handledelete(sep_handle(w3)) 'trouble deleting handles';
        sep_handle(w3) = 0;
...
parameter
gap_closed(iter);
gap_closed(iter) = (sum(w, lpsub_obj(iter,w))-sum(w,bender_sub_obj(iter,w)))/max (1e-3, (UB_Bender(iter)-sum(w,bender_sub_obj(iter,w))));
parameter
WallTime;
