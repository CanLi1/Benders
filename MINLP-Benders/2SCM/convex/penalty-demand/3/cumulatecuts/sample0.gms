...
equations
f1,f2,f3,f4,d1, d2,d3,d5,d6,d7,d8,d9,d10,d11,d12,d12p,d14p,d14,d15,d16,d17,de1,de3,de4,de5,de6,de7,de8,de9,de10,de11,de12,ds1,ds2,ds3,ds4,dobj;
...
n1p(iiter,r,p,j,w)$(freeze(w) and citer(iiter) ) .. aPU(r,p,j,w) =g= (lpPU(r,p,j,w) - PUhat(iiter,r,p,j,w))/PUU;
n1m(iiter,r,p,j,w)$(freeze(w)  and citer(iiter) ) .. aPU(r,p,j,w) =g= -(lpPU(r,p,j,w) - PUhat(iiter,r,p,j,w))/PUU;
n2p(iiter,p,c,j,w)$(freeze(w)  and citer(iiter) ) .. aF(p,c,j,w) =g= (lpF(p,c,j,w)-Fhat(iiter,p,c,j,w))/FUU;
n2m(iiter,p,c,j,w)$(freeze(w)  and citer(iiter) ) .. aF(p,c,j,w) =g= -(lpF(p,c,j,w)-Fhat(iiter,p,c,j,w))/FUU;
n3p(iiter,p,i,j,s,w)$(freeze(w) and JM(i,s,j)  and citer(iiter) and PS(i,s) and (ord(i) ne 4 or ord(j) ne 5 )) .. atheta(p,i,j,s,w)=g= (lptheta(p,i,j,s,w) - thetahat(iiter,p,i,j,s,w))/QEU(p,i)/100;
n3m(iiter,p,i,j,s,w)$(freeze(w) and JM(i,s,j)  and citer(iiter) and PS(i,s) and (ord(i) ne 4 or ord(j) ne 5 )) .. atheta(p,i,j,s,w)=g= -(lptheta(p,i,j,s,w) - thetahat(iiter,p,i,j,s,w))/QEU(p,i)/100;
n4p(iiter,p,i,j,s,w)$(freeze(w) and JM(i,s,j)  and citer(iiter) and PS(i,s) and (ord(i) eq 4 and ord(j) eq 5 )) .. atheta(p,i,j,s,w)=g= (lptheta(p,i,j,s,w) - thetahat(iiter,p,i,j,s,w))/QEU(p,i)/5;
n4m(iiter,p,i,j,s,w)$(freeze(w) and JM(i,s,j)  and citer(iiter) and PS(i,s) and (ord(i) eq 4 and ord(j) eq 5 )) .. atheta(p,i,j,s,w)=g= -(lptheta(p,i,j,s,w) - thetahat(iiter,p,i,j,s,w))/QEU(p,i)/5;
n5p(iiter,p,i,j,s,w)$(freeze(w)  and citer(iiter) and  (L(i,s,j) or Lbar(i,s,j)) and PS(i,s)) .. aWW(p,i,j,s,w) =g= (lpWW(p,i,j,s,w)-WWhat(iiter,p,i,j,s,w))/QEU(p,i)/100;
n5m(iiter,p,i,j,s,w)$(freeze(w)  and citer(iiter) and (L(i,s,j) or Lbar(i,s,j)) and PS(i,s)) .. aWW(p,i,j,s,w) =g= -(lpWW(p,i,j,s,w)-WWhat(iiter,p,i,j,s,w))/QEU(p,i)/100;
n6p(iiter,c,j,w)$(freeze(w)  and citer(iiter) and (ord(j)=3 or ord(j)=5)) .. aSlack(c,j,w) =g= (Slack(c,j,w) - Slackhat(iiter,c,j,w))/D(c,j,w);
n6m(iiter,c,j,w)$(freeze(w)  and citer(iiter) and (ord(j)=3 or ord(j)=5)) .. aSlack(c,j,w) =g= -(Slack(c,j,w) - Slackhat(iiter,c,j,w))/D(c,j,w);
n7p(iiter,r,p,w)$(freeze(w)  and citer(iiter)) .. ay(r,p,w) =g= lpy(r,p,w)-yhat(iiter,r,p,w);
n7m(iiter,r,p,w)$(freeze(w)  and citer(iiter)) .. ay(r,p,w) =g= -lpy(r,p,w)+yhat(iiter,r,p,w);
n8p(iiter,p,c,w)$(freeze(w)  and citer(iiter)) .. az(p,c,w) =g= (lpz(p,c,w)-zhat(iiter,p,c,w));
n8m(iiter,p,c,w)$(freeze(w)  and citer(iiter)) .. az(p,c,w) =g= -(lpz(p,c,w)-zhat(iiter,p,c,w));
n9p(iiter,p,i)$citer(iiter) .. ax(p,i) =g= lpx(p,i) - xhat(iiter, p,i);
n9m(iiter,p,i)$citer(iiter) .. ax(p,i) =g= xhat(iiter, p,i ) - lpx(p,i);
n10p(iiter,p,i)$citer(iiter) .. aQE(p,i) =g= lpQE(p,i) - QEhat(iiter, p,i);
n10m(iiter,p,i)$citer(iiter) .. aQE(p,i) =g= QEhat(iiter, p,i) - lpQE(p,i);
*define seperation problem obj
dobj(w)$(freeze(w)) .. dnorm =e=sum((p,i), ax(p,i)) + sum((p,i), aQE(p,i)) +  sum((r,p,j)$RJ(r,j), aPU(r,p,j,w)) + sum((p,c,j), aF(p,c,j,w)) + sum((p,i,j,s)$(JM(i,s,j) and PS(i,s) and (ord(i) ne 4 or ord(j) ne 5 )), atheta(p,i,j,s,w) )+ sum((p,i,j,s)$(JM(i,s,j) and PS(i,s) and (ord(i) eq 4 and ord(j) eq 5 )), atheta(p,i,j,s,w) ) + sum((p,i,j,s)$((L(i,s,j) or Lbar(i,s,j)) and PS(i,s)), aWW(p,i,j,s,w)) + sum((c,j)$(ord(j)=3 or ord(j)=5), aSlack(c,j,w) ) + sum((r,p), ay(r,p,w)) +sum((p,c), az(p,c,w));

model sep /f1,f2,f3,f4,d1, d2,d3,d5,d6,d7,d8,d9,d10,d11,d12,d12p,d14p,d14,d15,d16,d17,de1,de3,de4,de5,de6,de7,de8,de9,de10,de11,de12,ds1,ds2,ds3,ds4,dobj,n1p,n1m,n2p,n2m,n3p,n3m,n4p,n4m,n5p,n5m,n6p,n6m,n7p,n7m,n8p,n8m,n9p, n9m, n10p, n10m/;
...
equations
lp1, lp2;
lp1(iiter,rr,pp,w)$(freeze(w) and aRP(iiter,rr,pp,w)) .. sum((p,i), sign(xrp(iiter, p,i,w,rr,pp) - xhat(iiter, p,i)) * (x(p,i) - xrp(iiter, p,i,w, rr,pp ))) + sum((p,i), sign(QErp(iiter, p,i,w,rr,pp) - QEhat(iiter,p,i)) * (QE(p,i)-QErp(iiter, p,i, w,rr,pp)))+sum((r,p,j)$RJ(r,j), sign((PUrp(iiter,r,p,j,w,rr,pp)-PUhat(iiter,r,p,j,w))/PUU)*(PU(r,p,j,w) - PUrp(iiter,r,p,j,w,rr,pp))/PUU) + sum((p,c,j), sign((Frp(iiter,p,c,j,w,rr,pp)-Fhat(iiter,p,c,j,w))/FUU)*(F(p,c,j,w)-Frp(iiter,p,c,j,w,rr,pp))/FUU) + sum((p,i,j,s)$(JM(i,s,j) and PS(i,s)and (ord(i) ne 4 or ord(j) ne 5 )), sign((thetarp(iiter,p,i,j,s,w,rr,pp)-thetahat(iiter,p,i,j,s,w))/QEU(p,i)/100)*(theta(p,i,j,s,w) - thetarp(iiter,p,i,j,s,w,rr,pp))/QEU(p,i)/100) + sum((p,i,j,s)$(JM(i,s,j) and PS(i,s)and (ord(i) eq 4 and ord(j) eq 5 )), sign((thetarp(iiter,p,i,j,s,w,rr,pp)-thetahat(iiter,p,i,j,s,w))/QEU(p,i)/5)*(theta(p,i,j,s,w) - thetarp(iiter,p,i,j,s,w,rr,pp))/QEU(p,i)/5)+ sum((p,i,j,s)$((L(i,s,j) or Lbar(i,s,j)) and PS(i,s)), sign((WWrp(iiter,p,i,j,s,w,rr,pp)-WWhat(iiter,p,i,j,s,w))/QEU(p,i)/100)*(WW(p,i,j,s,w)-WWrp(iiter,p,i,j,s,w,rr,pp))/QEU(p,i)/100) + sum((c,j)$(ord(j)=3 or ord(j)=5), sign((Slackrp(iiter,c,j,w,rr,pp)-Slackhat(iiter,c,j,w))/D(c,j,w))*(Slack(c,j,w) - Slackrp(iiter,c,j,w,rr,pp))/D(c,j,w)) + sum((r,p), sign((yrp(iiter,r,p,w,rr,pp)-yhat(iiter,r,p,w)))*(y(r,p,w)-yrp(iiter,r,p,w,rr,pp))) +sum((p,c), sign((zrp(iiter,p,c,w,rr,pp)-zhat(iiter,p,c,w)))*(z(p,c,w)-zrp(iiter,p,c,w,rr,pp))) =g= 0;
lp2(iiter,pp,cc,w)$(freeze(w) and aPC(iiter,pp,cc,w)) .. sum((p,i), sign(xpc(iiter, p,i,w,pp,cc) - xhat(iiter, p,i)) * (x(p,i) - xpc(iiter, p,i, w,pp,cc ))) + sum((p,i), sign(QEpc(iiter, p,i,w,pp,cc) - QEhat(iiter,p,i)) * (QE(p,i)-QEpc(iiter, p,i, w,pp,cc)))+sum((r,p,j)$RJ(r,j), sign((PUpc(iiter,r,p,j,w,pp,cc)-PUhat(iiter,r,p,j,w))/PUU)*(PU(r,p,j,w) - PUpc(iiter,r,p,j,w,pp,cc))/PUU) + sum((p,c,j), sign((Fpc(iiter,p,c,j,w,pp,cc)-Fhat(iiter,p,c,j,w))/FUU)*(F(p,c,j,w)-Fpc(iiter,p,c,j,w,pp,cc))/FUU) + sum((p,i,j,s)$(JM(i,s,j) and PS(i,s)and (ord(i) ne 4 or ord(j) ne 5 )), sign((thetapc(iiter,p,i,j,s,w,pp,cc)-thetahat(iiter,p,i,j,s,w))/QEU(p,i)/100)*(theta(p,i,j,s,w) - thetapc(iiter,p,i,j,s,w,pp,cc))/QEU(p,i)/100)+ sum((p,i,j,s)$(JM(i,s,j) and PS(i,s)and (ord(i) eq 4 and ord(j) eq 5 )), sign((thetapc(iiter,p,i,j,s,w,pp,cc)-thetahat(iiter,p,i,j,s,w))/QEU(p,i)/5)*(theta(p,i,j,s,w) - thetapc(iiter,p,i,j,s,w,pp,cc))/QEU(p,i)/5)+ sum((p,i,j,s)$((L(i,s,j) or Lbar(i,s,j)) and PS(i,s)), sign((WWpc(iiter,p,i,j,s,w,pp,cc)-WWhat(iiter,p,i,j,s,w))/QEU(p,i)/100)*(WW(p,i,j,s,w)-WWpc(iiter,p,i,j,s,w,pp,cc))/QEU(p,i)/100) + sum((c,j)$(ord(j)=3 or ord(j)=5), sign((Slackpc(iiter,c,j,w,pp,cc)-Slackhat(iiter,c,j,w))/D(c,j,w))*(Slack(c,j,w) - Slackpc(iiter,c,j,w,pp,cc))/D(c,j,w)) + sum((r,p), sign((ypc(iiter,r,p,w,pp,cc)-yhat(iiter,r,p,w)))*(y(r,p,w)-ypc(iiter,r,p,w,pp,cc))) +sum((p,c), sign((zpc(iiter,p,c,w,pp,cc)-zhat(iiter,p,c,w)))*(z(p,c,w)-zpc(iiter,p,c,w,pp,cc))) =g= 0;
...
*update parameters
	xrp(iter, p,i, w3, r3, p3) = lpx.l(p,i);
	QErp(iter,p,i,w3, r3,p3) = lpQE.l(p,i);
          PUrp(iter,r,p,j,w3,r3,p3) = lpPU.l(r,p,j,w3);
        Frp(iter,p,c,j,w3,r3,p3) = lpF.l(p,c,j,w3);
        thetarp(iter,p,i,j,s,w3,r3,p3) = lptheta.l(p,i,j,s,w3);
        WWrp(iter,p,i,j,s,w3,r3,p3) = lpWW.l(p,i,j,s,w3);
        Slackrp(iter,c,j,w3,r3,p3) = lpSlack.l(c,j,w3);
        yrp(iter,r,p,w3,r3,p3) = lpy.l(r,p,w3);
        zrp(iter,p,c,w3,r3,p3) = lpz.l(p,c, w3);
  ...
  loop(c3,
    ...
    Repeat
      loop(w3$handlecollect(sep_handle(w3)),
        cpu_sep = cpu_sep + sep.resusd;
*update parameters
		xpc(iter, p,i,w3, p3,c3) = lpx.l(p,i);
		QEpc(iter, p,i,w3, p3,c3) = lpQE.l(p,i);
          PUpc(iter,r,p,j,w3,p3,c3) = lpPU.l(r,p,j,w3);
        Fpc(iter,p,c,j,w3,p3,c3) = lpF.l(p,c,j,w3);
        thetapc(iter,p,i,j,s,w3,p3,c3) = lptheta.l(p,i,j,s,w3);
        WWpc(iter,p,i,j,s,w3,p3,c3) = lpWW.l(p,i,j,s,w3);
        Slackpc(iter,c,j,w3,p3,c3) = lpSlack.l(c,j,w3);
        ypc(iter,r,p,w3,p3,c3) = lpy.l(r,p,w3);
        zpc(iter,p,c,w3,p3,c3) = lpz.l(p,c, w3);
        display sep.modelStat;
        if(dnorm.l lt 1e-2 or (sep.modelStat ne 2 and sep.modelStat ne 1),
