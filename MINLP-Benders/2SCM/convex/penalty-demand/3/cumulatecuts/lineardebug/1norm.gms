...
model sep /f1,f2,f3,f4,d1, d2,d3,d5,d6,d7,d8,d9,d10,d11,d12,d14,d12p,d14p,d15,d16,d17,de1,de3,de4,de5,de6,de7,de8,de9,de10,de11,de12,delp1,delp2,ds1,ds2,ds3,ds4,n1p,n1m,n2p,n2m,n3p,n3m,n4p,n4m,n5p,n5m,n6p,n6m,n7p,n7m,n8p,n8m,n9p, n9m, n10p, n10m,dobj/;
*--------------DEFINE subproblem with lift and project cuts---------------------




*check if the cuts if valid by plug in the optimal solution
parameters
ox(p,i), oQE(p,i),oPU(r,p,j,w),oF(p,c,j,w),otheta(p,i,j,s,w),oWW(p,i,j,s,w),oSlack(c,j,w),oy(r,p,w),oz(p,c,w);
variables
stat1(iter,r,p,w), stat2(iter,p,c,w),stat3;
equations 
clp1,clp2,clp3;
clp1(iiter,rr,pp,w)$(freeze(w) and aRP(iiter,rr,pp,w)) .. sum((p,i), xrp(iiter, p,i,w,rr,pp)* ox(p,i)) + sum((p,i), QErp(iiter, p,i,w,rr,pp)  * oQE(p,i))+sum((r,p,j)$RJ(r,j), PUrp(iiter,r,p,j,w,rr,pp)*oPU(r,p,j,w) ) + sum((p,c,j), Frp(iiter,p,c,j,w,rr,pp)*oF(p,c,j,w)) + sum((p,i,j,s)$(JM(i,s,j) and PS(i,s)and (ord(i) ne 4 or ord(j) ne 5 )), thetarp(iiter,p,i,j,s,w,rr,pp)*otheta(p,i,j,s,w) ) + sum((p,i,j,s)$(JM(i,s,j) and PS(i,s)and (ord(i) eq 4 and ord(j) eq 5 )), thetarp(iiter,p,i,j,s,w,rr,pp)*otheta(p,i,j,s,w) )+ sum((p,i,j,s)$((L(i,s,j) or Lbar(i,s,j)) and PS(i,s)), WWrp(iiter,p,i,j,s,w,rr,pp)*oWW(p,i,j,s,w)) + sum((c,j)$(ord(j)=3 or ord(j)=5), Slackrp(iiter,c,j,w,rr,pp)*oSlack(c,j,w) ) + sum((r,p), yrp(iiter,r,p,w,rr,pp)*oy(r,p,w)) +sum((p,c), zrp(iiter,p,c,w,rr,pp)*oz(p,c,w)) + drpconst(iiter,w,rr,pp)=e= stat1(iiter,rr,pp,w);
clp2(iiter,pp,cc,w)$(freeze(w) and aPC(iiter,pp,cc,w)) .. sum((p,i), xpc(iiter, p,i,w,pp,cc)* ox(p,i)) + sum((p,i), QEpc(iiter, p,i,w,pp,cc)  * oQE(p,i))+sum((r,p,j)$RJ(r,j), PUpc(iiter,r,p,j,w,pp,cc)*oPU(r,p,j,w) ) + sum((p,c,j), Fpc(iiter,p,c,j,w,pp,cc)*oF(p,c,j,w)) + sum((p,i,j,s)$(JM(i,s,j) and PS(i,s)and (ord(i) ne 4 or ord(j) ne 5 )), thetapc(iiter,p,i,j,s,w,pp,cc)*otheta(p,i,j,s,w) ) + sum((p,i,j,s)$(JM(i,s,j) and PS(i,s)and (ord(i) eq 4 and ord(j) eq 5 )), thetapc(iiter,p,i,j,s,w,pp,cc)*otheta(p,i,j,s,w) )+ sum((p,i,j,s)$((L(i,s,j) or Lbar(i,s,j)) and PS(i,s)), WWpc(iiter,p,i,j,s,w,pp,cc)*oWW(p,i,j,s,w)) + sum((c,j)$(ord(j)=3 or ord(j)=5), Slackpc(iiter,c,j,w,pp,cc)*oSlack(c,j,w) ) + sum((r,p), ypc(iiter,r,p,w,pp,cc)*oy(r,p,w)) +sum((p,c), zpc(iiter,p,c,w,pp,cc)*oz(p,c,w)) +dpcconst(iiter,w,pp,cc) =e= stat2(iiter,pp,cc,w);
clp3 .. stat3 =e= 1;
model checkstat /clp1,clp2,clp3/;
option optcr = 0;
option optca =0;
model fullspace /e1,e2,e3,e4,e5,e6,e7,e8,e9,e10,e11,e12, eobj/;
freeze(w) = yes;
solve fullspace using mip minimizing cost;
ox(p,i)=x.l(p,i);
oQE(p,i)=QE.l(p,i);
oPU(r,p,j,w)=PU.l(r,p,j,w);
oF(p,c,j,w)=F.l(p,c,j,w);
otheta(p,i,j,s,w)=theta.l(p,i,j,s,w);
oWW(p,i,j,s,w)=WW.l(p,i,j,s,w);
oSlack(c,j,w)=Slack.l(c,j,w);
oy(r,p,w)=y.l(r,p,w);
oz(p,c,w)=z.l(p,c,w);
*-------------------solve model -----------------------------------
...
*solve NLP relaxation--------------
  loop(w3,
    ...
    Slack.l(c,'j3', w3) = 50;
    Slack.l(c,'j5',w3) = 2;
    solve lpsub using rMIP minimizing COST;
...
display bender_sub_modelstat,upper_sub_modelstat,master_modelstat,bender_sub_obj, lpsub_obj,ub_obj, gap_closed, BenderOBJ_record, UB_Bender,UB, LB, WallTime,cpu_ub, cpu_bender_sub, cpu_bender_master, cpu_sep, cpu_lpsub ;
display baditer;
display stat1.l, stat2.l;
